# app_streamlit.py —— 合并版（未来班次过滤 + 30s 刷新 + Bus 折叠 + 稳定线路几何 + 官方配色）
...
# route 列表
SUBWAY_ID = (
    DATAFRAMES.get("subway", pd.DataFrame())
    .get("route_id", pd.Series([], dtype=object))
    .unique()
)
LIRR_ROUTE = (
    DATAFRAMES.get("LIRR", pd.DataFrame())
    .get("route_id", pd.Series([], dtype=object))
    .unique()
)
BUS_ROUTE_MAPPING = {
    "Bronx": DATAFRAMES.get("bus_bronx", pd.DataFrame()).get(
        "route_id", pd.Series([], dtype=object)
    ).unique(),
    "Brooklyn": DATAFRAMES.get("bus_brooklyn", pd.DataFrame()).get(
        "route_id", pd.Series([], dtype=object)
    ).unique(),
    "Manhattan": DATAFRAMES.get("bus_manhattan", pd.DataFrame()).get(
        "route_id", pd.Series([], dtype=object)
    ).unique(),
    "Queens": DATAFRAMES.get("bus_queens", pd.DataFrame()).get(
        "route_id", pd.Series([], dtype=object)
    ).unique(),
    "Staten_Island": DATAFRAMES.get("bus_staten_island", pd.DataFrame()).get(
        "route_id", pd.Series([], dtype=object)
    ).unique(),
    "New_Jersy": DATAFRAMES.get("bus_new_jersy", pd.DataFrame()).get(
        "route_id", pd.Series([], dtype=object)
    ).unique(),
}
...
def precompute_route_lines(key: str) -> dict[str, list[pd.DataFrame]]:
    """
    为指定数据集（subway / LIRR / bus_xxx）构造：
      route_id -> [segment_df, ...]

    思路：
      - 每个 route 选停站最多的 trip 做代表（简单 + 快）
      - 按 stop_sequence diff 切成若干段
      - 去掉重复坐标
      - 丢掉太短的段（<=2 个点）
    """
...
BUS_LINES = {
    b: precompute_route_lines(f"bus_{b.lower()}")
    for b in BOROUGHS
    if f"bus_{b.lower()}" in DATAFRAMES
}
...
with st.sidebar:
    ...
    if map_choice == "subway":
        ...
        bus_routes = sorted(
            [str(r) for r in BUS_ROUTE_MAPPING.get(_borough, []) if pd.notna(r)],
            key=str,
        )
    ...
    with cols[0]:
        if st.button("🔄 Refresh now"):
            # 只清实时 & citibike cache，不动静态 GTFS / 线路几何
